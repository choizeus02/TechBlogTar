...
import com.choizeus.techblogtar.Repository.UserRepository;
...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        Users users = (Users) userRepository.findByEmail(email)
                .orElseThrow(() -> new UsernameNotFoundException("해당하는 유저가 없습니다."));

        CustomUserInfoDto customUserInfoDto = new CustomUserInfoDto(users.getId(), users.getEmail(), users.getPassword(), users.getNickname());
